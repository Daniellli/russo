...
class SamplingModule(nn.Module):
    ...
    """
    def __init__(self, sampling_method, num_proposal, feat_dim, lang_dim):
        super().__init__()
        ...
        if sampling_method == 'fps':
            ...
            self.match_module = MatchModule(object_dim=feat_dim, lang_dim=lang_dim, fusion_dim=256)
            #!===========
            self.gsample_module = GeneralSamplingModule()
        else:
...
class GeneralSamplingModule(nn.Module):
    ...
    def forward(self, xyz, features, sample_inds):
        ...
        Args:
            ...
            features: (B,C,K)
        """
        xyz_flipped = xyz.transpose(1, 2).contiguous()#* 根据sample_inds 对xyz 和feature 采样 
        new_xyz = pointnet2_utils.gather_operation(xyz_flipped, sample_inds).transpose(1, 2).contiguous()
        new_features = pointnet2_utils.gather_operation(features, sample_inds).contiguous()
...
class SaSamplingModule(nn.Module):
    ...
    def forward(self, xyz, features, sample_inds=None):
        ...
        Args:
            ...
            features: (B,C,K)
        """
        new_xyz, new_features, new_sample_inds = self.sa_module(xyz, features, sample_inds)#*  根据sample_inds 从xyz and features   采样
